...
async function run(viewport,label){
  ...
  if((await page.locator('.todayContextGrid > article').count())!==4)throw new Error(`${label}: recent-context grid should have four evidence cards`);
  const text=(await page.locator('.todayContextSection').textContent())||'';
  for(const expected of ['Contexto recente','peso +1,0 kg','MME +1,0 kg','gordura -1,3 p.p.','Duas sessões mais recentes','3 tipo(s) de métrica em 02/02/2026','2 resultado(s) na coleta mais recente']){
