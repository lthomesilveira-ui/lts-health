...
assert.equal(uxCoherence?.status,'done');
const executiveDensity=state.tasks.find(task=>task.id==='LTS-UX-DENSITY-002');
...
for(const id of state.current_package.task_ids)assert.ok(state.tasks.some(t=>t.id===id),'package references unknown task');
assert.ok(state.package_history.some(p=>p.id==='PKG-EXECUTIVE-DENSITY-002'),'historical package is preserved');
...
assert.equal(visualSource?.status,'done');
assert.equal(visualSource?.blocker,null);
assert.ok(visualSource.evidence.some(x=>x.includes('Private Drive')),'recovered reference must have private durable location');
assert.ok(visualSource.evidence.some(x=>x.includes('dc322921')),'recovered source identity is preserved');
