...
await page.evaluate(async()=>{
  ...
  core.state.data={
    body:[
      {measured_at:'2026-07-16',weight_kg:99.8,body_fat_pct:13.6,skeletal_muscle_mass_kg:50.3,source:'InBody'},
      {measured_at:'2026-08-24',weight_kg:98.6,body_fat_pct:11.6,skeletal_muscle_mass_kg:50.6,source:'InBody'}
    ],
    workouts:[
...
const fat=result.change.find(x=>x.label==='Gordura'),muscle=result.change.find(x=>x.label==='Massa muscular');
...
if(!muscle||muscle.value==='—'||!muscle.detail.includes('medição anterior'))throw new Error(`Muscle change does not reuse current-composition comparison: ${JSON.stringify(muscle)}`);
