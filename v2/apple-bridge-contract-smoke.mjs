...
for(const token of [
  ...
  if(!fn.includes(token) && token!=='verify_jwt')throw new Error(`Apple bridge contract missing: ${token}`);
}
...
if(/source_payload[^\n]*metric_date/.test(backup))throw new Error('backup source metric loader still appears to include raw source payload');
