...
async function run(viewport,label){
  ...
  for(const expected of ['assistente longitudinal','Composição corporal','Treinos','Nutrição','Recuperação','Exames','Evolução','Peso corporal','Resumo da janela','O que mudou','Últimos acontecimentos','Dados a completar'])if(!text.includes(expected))throw new Error(`${label}: longitudinal home missing ${expected}`);
  ...
  const audit=await page.evaluate(async()=>{
    ...
    const data={
      body:[{measured_at:'2026-08-01',weight_kg:100,skeletal_muscle_mass_kg:49,fat_mass_kg:14,body_fat_pct:14,source:'same'},{measured_at:'2026-08-20',weight_kg:99,skeletal_muscle_mass_kg:49.5,fat_mass_kg:13.2,body_fat_pct:13.3,source:'same'}],
