...
function events(){
  ...
  if(!failed('activity'))for(const a of state.data.activity||[]){
  ...
  if(!failed('metrics'))for(const m of state.data.metrics||[]){
...
export function renderTimelineHub(){
  const all=events(),missing=unavailable(),domain=state.ui.timelineDomain,q=norm(state.ui.timelineQuery),domains=['all',...unique(all.map(e=>e.domain)).sort((a,b)=>a.localeCompare(b,'pt-BR'))];
  ...
  return `${title('Timeline','Seu histórico em ordem de data. Atividade geral e treinos continuam separados para evitar dupla contagem.')}
    ...
    <div class="controls sectionGap"><select id="timelineDomain">${domains.map(d=>`<option value="${esc(d)}">${d==='all'?'Todas as áreas':esc(d)}</option>`).join('')}</select><input id="timelineQuery" type="search" placeholder="Buscar no histórico" value="${esc(state.ui.timelineQuery)}"></div>
