...
export function renderTraceabilityPanel(model){
  ...
  return `<section class="card evidenceTraceability sectionGap" data-evidence-traceability>
    ...
    <div class="evidenceTraceabilityFlow">
      ...
      <div><i>3</i><span><b>Estrutura e proveniência</b><small>${model?.preservedSeries==null?'cobertura complementar indisponível agora':`${model.structuredDomains} domínio(s) com dados · ${model.preservedSeries} série(s) candidate/held mantidas separadas${model.workoutEvidence==null?'':` · ${model.workoutEvidence} vínculo(s) complementares de treino`}`}</small></span></div>
