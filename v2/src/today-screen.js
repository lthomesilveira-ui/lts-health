...
export function renderTodayHub(){
  ...
  return`<div class="dashboardScreen cockpitScreen cockpitV3" data-executive-dashboard data-period="${esc(period)}">
    ...
    <section class="cockpitSecondaryGrid">
      ...
      <article class="cockpitModule hydration ${model.water.length?'':'missing'}"><div class="cockpitModuleHead"><div><span>Hidratação</span><h2>Ingestão de água</h2></div><button class="cockpitButton" data-entry="water-import">${model.water.length?'Atualizar MFP':'Importar do MFP'}</button></div>${model.water.length?`<div class="cockpitHydrationValue"><b>${fmtNum(model.water.at(-1).value,0)} mL</b><span>último registro em ${fmtDate(model.water.at(-1).date)} · ${esc(model.water.at(-1).source||'MyFitnessPal')}</span></div>`:`<div class="cockpitHydrationMissing"><span>◌</span><b>Sem dado de ingestão de água</b><p>Use a importação histórica do MyFitnessPal para trazer o período em um único arquivo, sem digitar dia a dia. Água corporal da bioimpedância é outra medida e aparece em Composição.</p></div>`}</article>
    </section>
