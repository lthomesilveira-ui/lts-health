...
export function renderTodayHub(){
  ...
  const labCard=domainFailed('labs')?currentCard('Exames','Indisponível agora','Os resultados laboratoriais não carregaram nesta atualização.','saude'):model.labs.reason==='ambiguous_source'?currentCard('Exames','Revisão necessária','Há mais de uma origem nas coletas recentes; nenhuma foi escolhida para comparação.','saude'):currentCard('Exames',labDate?fmtDate(labDate):'Sem coleta estruturada',model.labs.collectionDays?.length>=2?`${model.labs.comparable} biomarcador(es) comparáveis na última dupla de coletas`:'Ainda sem segunda coleta comparável','saude');
