...
export function activitySleepSnapshot(data={},status={}){
  ...
  const energySeries=dateRows(activityRows.filter(r=>r.metric_type==='active_energy_kcal'),'measured_at').filter(([,items])=>items.length===1).map(([date,items])=>({date,value:num(items[0].value)})).filter(r=>r.value!=null).slice(-30);
  ...
  const sleepSources=[...sourceMap.values()].map(source=>{
    ...
    return{key:source.key,label:source.label,family:source.family,days:safe.length,lastDate:latest?.date||null,lastValue:num(latest?.row?.value),unit:String(latest?.row?.unit||'h').trim()||'h'};
...
function activitySleepPanel(snapshot){
  if(!snapshot.metricsReady&&!snapshot.sourceReady)return`<section class="dashboardPanel cockpitActivitySleep"><div class="dashboardPanelHead"><div><span>Atividade & sono</span><h2>Cobertura por origem</h2></div><button data-route="analise">Insights →</button></div><div class="dashboardChartEmpty">Atividade e sono não carregaram agora.</div></section>`;
  const activityFacts=snapshot.activityLatest?`<div class="cockpitActivityFacts"><div><span>Energia ativa</span><b>${snapshot.activeEnergy.row?`${fmtNum(snapshot.activeEnergy.row.value,0)} kcal`:'—'}</b><small>${snapshot.activeEnergy.date?fmtDate(snapshot.activeEnergy.date):'sem ponto inequívoco'}</small></div><div><span>Exercício</span><b>${snapshot.exercise.row?`${fmtNum(snapshot.exercise.row.value,0)} min`:'—'}</b><small>${snapshot.exercise.date?fmtDate(snapshot.exercise.date):'sem ponto inequívoco'}</small></div><div><span>Em pé</span><b>${snapshot.stand.row?`${fmtNum(snapshot.stand.row.value,0)} h`:'—'}</b><small>${snapshot.stand.date?fmtDate(snapshot.stand.date):'sem ponto inequívoco'}</small></div></div>`:`<div class="cockpitMissing compact"><b>Sem atividade diária confirmada.</b><p>O cockpit só usa as métricas diárias autorizadas da Apple; outras medições permanecem fora deste resumo.</p></div>`;
  const sleep=snapshot.sleepSources.length?`<div class="cockpitSleepSources"><div class="cockpitSubhead"><b>Sono preservado por origem</b><small>${snapshot.sleepSources.length} origem(ns); sem média entre fontes</small></div>${snapshot.sleepSources.slice(0,4).map(source=>`<div class="cockpitSleepRow"><div><b>${esc(source.label)}</b><small>${source.days} dia(s) inequívoco(s)</small></div><span>${source.lastValue!=null?`${fmtNum(source.lastValue,1)} ${esc(source.unit)}`:'—'}<small>${source.lastDate?fmtDate(source.lastDate):'—'}</small></span></div>`).join('')}<p class="dashboardFootnote">Fontes com datas sobrepostas continuam separadas. Nenhuma é somada ou tratada automaticamente como continuação de outra.</p></div>`:`<div class="cockpitMissing compact"><b>Sono sem evidência estruturada por origem.</b><p>Quando houver registros preservados, cada origem aparecerá separadamente.</p></div>`;
  return`<section class="dashboardPanel cockpitActivitySleep"><div class="dashboardPanelHead"><div><span>Atividade & sono</span><h2>Cobertura por origem</h2></div><button data-route="analise">Abrir Insights →</button></div><div class="cockpitActivitySleepGrid"><div><div class="cockpitSubhead"><b>Atividade diária confirmada</b><small>${snapshot.activityLatest?`até ${fmtDate(snapshot.activityLatest)}`:'sem cobertura confirmada'}</small></div>${activityFacts}${snapshot.energySeries.length>=2?scaledChart(snapshot.energySeries,{unit:' kcal',digits:0,label:'Energia ativa diária confirmada',bar:true}):''}</div>${sleep}</div></section>`;
}
function insightCards(model,activitySleep){
  ...
  if(activitySleep.activityLatest||activitySleep.sleepSources.length)cards.push(noteCard('Atividade & sono',activitySleep.sleepSources.length?`Sono preservado em ${activitySleep.sleepSources.length} origem(ns)`:'Atividade diária confirmada',`${activitySleep.activityLatest?`Atividade confirmada até ${fmtDate(activitySleep.activityLatest)}. `:''}${activitySleep.sleepLatest?`Sono preservado por origem até ${fmtDate(activitySleep.sleepLatest)}. `:''}Fontes de sono não são consolidadas entre si.`,'analise'));
...
export function renderTodayHub(){
  const model=buildIntegratedAnalysis(state.data,state.domainStatus),body=latestBodySnapshot(),workout=model.training.lastWorkout,nutrition={date:model.lastNutritionDate,row:model.lastNutrition,ambiguous:model.nutritionLatestAmbiguous},labs=labStats(),protocol=protocolStats(),water=hydrationRows(),activitySleep=activitySleepSnapshot(state.data,state.domainStatus),reference=unique([model.referenceDay,protocol.last,water.at(-1)?.date,activitySleep.latestDate].filter(Boolean)).sort().at(-1)||null;
  ...
  return`<div class="dashboardScreen cockpitScreen" data-executive-dashboard>
    <section class="dashboardHeader cockpitHero"><div><span class="dashboardEyebrow">Cockpit LTS Health</span><h1>Seu estado de saúde em uma tela</h1><p>${reference?`Dados estruturados disponíveis até ${fmtDate(reference)}.`:'Ainda sem data de referência.'} Composição, treino, nutrição, hidratação, atividade & sono, exames e protocolos ficam lado a lado, com lacunas explícitas e sem preencher dados por estimativa.</p></div><div class="dashboardHeaderActions">${action('analise','Abrir Insights')}${action('dados','Adicionar dados')}</div></section>
    ...
    <section class="dashboardSection cockpitSection"><div class="dashboardSectionHead"><div><span>Visão executiva</span><h2>Evolução e cobertura</h2></div><small>Escalas, datas e origens explícitas</small></div><div class="cockpitDashboardGrid">${compositionPanel(model)}${trainingPanel()}${hydrationPanel()}${labPanel()}${activitySleepPanel(activitySleep)}</div></section>
    <section class="dashboardSection cockpitSection"><div class="dashboardSectionHead"><div><span>Insights</span><h2>Leituras sustentadas pelos dados</h2></div><button class="todayAction" data-route="analise">Ver todos</button></div><div class="dashboardInsightGrid cockpitInsightGrid">${insightCards(model,activitySleep)}</div></section>
    <p class="dashboardFootnote cockpitFooter">O LTS Health organiza e compara registros existentes. Sono permanece separado por origem, protocolos aparecem apenas como contexto temporal e os exames não recebem interpretação clínica automática.</p>
  </div>`;
}
