...
export function renderTodayHub(){
  ...
  const protocolReady=state.domainStatus?.treatments==='ready',protocolCard=failed('treatments')?routeCard('tratamentos','Protocolos','Indisponível agora','O histórico não carregou.','error'):protocolReady&&protocol.rows?routeCard('tratamentos','Protocolos',`${protocol.rows} registros`,`${protocol.names} item(ns) distintos · até ${fmtDate(protocol.last)}`):protocolReady?routeCard('tratamentos','Protocolos','Sem registros','Nenhum evento estruturado.'):routeCard('tratamentos','Protocolos','Abrir histórico','Eventos registrados por data e origem.');
