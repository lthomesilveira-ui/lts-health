...
function exerciseHistory(group){
  ...
  return `<div class="exerciseHistoryHead"><b>${esc(group.label)}</b><small>Cargas de unidades diferentes permanecem separadas.</small></div><div class="list">${days.map(date=>{
    ...
    return `<div class="row"><time>${fmtDate(date)}</time><div><b>${tops||'carga não estruturada'}</b><small>${sets.length} série(s) registradas</small></div></div>`;
...
export function renderTrainingScreen(){
  ...
  const rows=all.filter(w=>(!cut||day(w.workout_date)>=cut)&&(!query||norm(`${w.workout_type} ${w.location} ${(w.muscle_groups||[]).join(' ')}`).includes(query)));
