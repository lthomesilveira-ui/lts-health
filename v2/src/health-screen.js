import {state,esc,fmtDate,fmtNum,num,norm,unique,countLabel} from './core.js';
...
function comparableSeries(group){
  if(!group)return[];
  return sourceUnitCohorts(group.rows.filter(isNumericResult)).map(cohort=>({...cohort,...trendRows(cohort)})).filter(cohort=>cohort.rows.length>=2);
}
function markerSeriesSummary(group){
  const series=comparableSeries(group),points=series.map(s=>s.rows.length),dates=unique(series.flatMap(s=>s.rows.map(r=>r.collection_date).filter(Boolean))).sort();
  return{series,count:series.length,maxPoints:points.length?Math.max(...points):0,dateCount:dates.length,firstDate:dates[0]||null,lastDate:dates.at(-1)||null};
}
function longitudinalGroups(groups){
  return groups.map(group=>({group,summary:markerSeriesSummary(group)})).filter(item=>item.summary.maxPoints>=2).sort((a,b)=>b.summary.maxPoints-a.summary.maxPoints||b.summary.dateCount-a.summary.dateCount||a.group.label.localeCompare(b.group.label,'pt-BR'));
}
function chartTick(value,unit){return `${fmtNum(value)}${unit?` ${unit}`:''}`;}
function markerChart(ordered,unit,origin){
  ...
  const w=620,h=190,left=62,right=18,top=18,bottom=30,lo=Math.min(...values),hi=Math.max(...values),span=hi-lo||Math.max(Math.abs(hi),1),pad=span*.14,min=lo-pad,max=hi+pad,mid=(min+max)/2;
  const innerW=w-left-right,innerH=h-top-bottom,x=i=>left+i*innerW/Math.max(1,points.length-1),y=v=>top+(max-v)*innerH/(max-min||1);
  const path=points.map((pt,i)=>`${i?'L':'M'}${x(i).toFixed(1)} ${y(pt.value).toFixed(1)}`).join(' ');
  const dots=points.map((pt,i)=>`<circle cx="${x(i).toFixed(1)}" cy="${y(pt.value).toFixed(1)}" r="4"><title>${esc(fmtDate(pt.date))}: ${esc(fmtNum(pt.value))} ${esc(unit||'')}</title></circle>`).join('');
  const middlePoint=points[Math.floor((points.length-1)/2)];
  return `<div class="labHistoryChart"><div class="labHistoryChartHead"><b>Série histórica</b><small>${countLabel(points.length,'ponto','pontos')} · ${esc(origin)} · ${esc(unit||'sem unidade')}</small></div><svg viewBox="0 0 ${w} ${h}" role="img" aria-label="Série histórica do marcador selecionado"><path class="labHistoryGrid" d="M${left} ${y(max).toFixed(1)}H${w-right} M${left} ${y(mid).toFixed(1)}H${w-right} M${left} ${y(min).toFixed(1)}H${w-right}"/><text class="labHistoryYLabel" x="${left-8}" y="${(y(max)+4).toFixed(1)}" text-anchor="end">${esc(chartTick(max,unit))}</text><text class="labHistoryYLabel" x="${left-8}" y="${(y(mid)+4).toFixed(1)}" text-anchor="end">${esc(chartTick(mid,unit))}</text><text class="labHistoryYLabel" x="${left-8}" y="${(y(min)+4).toFixed(1)}" text-anchor="end">${esc(chartTick(min,unit))}</text><path class="labHistoryLine" d="${path}"/>${dots}</svg><div class="labHistoryAxis"><span>${fmtDate(points[0].date)}</span><span>${fmtDate(middlePoint.date)}</span><span>${fmtDate(points.at(-1).date)}</span></div><small class="labHistoryNote">Escala vertical mostra os valores registrados na mesma origem e unidade. O gráfico é descritivo: não classifica o resultado nem substitui interpretação clínica.</small></div>`;
}
...
function markerHistory(group){
  ...
  return `<div class="markerHead"><b>${esc(group.label)}</b><small>${countLabel(numeric.length,'resultado numérico','resultados numéricos')} · ${countLabel(textual.length,'resultado textual','resultados textuais')}</small></div>${trend}<details class="inlineDisclosure"><summary>Ver todos os resultados deste marcador</summary><div class="list labHistory">${history}</div></details>`;
}
function longitudinalOverview(items,selectedKey){
  if(!items.length)return `<div class="note">Ainda não há marcador com pelo menos dois pontos inequívocos da mesma origem e unidade.</div>`;
  const top=items.slice(0,12),max=items[0].summary.maxPoints;
  return `<div class="labSeriesOverview"><div class="labSeriesSummary"><div><span>Marcadores com série</span><b>${items.length}</b><small>mínimo de 2 pontos comparáveis</small></div><div><span>Maior série</span><b>${max} pontos</b><small>uma única origem e unidade</small></div></div><div class="labSeriesShortcuts" aria-label="Marcadores com série longitudinal">${top.map(({group,summary})=>`<button type="button" data-marker="${esc(group.key)}" class="${group.key===selectedKey?'active':''}"><b>${esc(group.label)}</b><small>${summary.maxPoints} pontos · ${fmtDate(summary.firstDate)} → ${fmtDate(summary.lastDate)}</small></button>`).join('')}</div><small class="labHistoryNote">Atalhos ordenados pela maior série disponível. Resultados de outra origem, outra unidade ou datas ambíguas não aumentam a série.</small></div>`;
}
...
function collectionComparison(cols,selectedKey){
  ...
  if(prior.reason==='source_gap')return `<div class="note">Há coletas anteriores, mas nenhuma da mesma origem de ${esc(current.lab)}. Nenhuma diferença foi calculada automaticamente; compare somente após confirmar que as origens são equivalentes.</div>`;
...
function evidenceByDate(cols,docs){
  ...
  return `<div class="evidenceDateList">${rows.map(x=>{
    ...
    const status=sameOrigin.length?`${countLabel(sameOrigin.length,'documento','documentos')} da mesma origem`:dateOnly.length?'Documentos só coincidem na data':'Origem do documento não informada';
    const resultCount=x.collections.reduce((s,c)=>s+c.rows.length,0);
    return `<article data-evidence-date="${esc(x.date)}"><time>${fmtDate(x.date)}</time><div><b>${countLabel(resultCount,'resultado','resultados')} · ${countLabel(x.docs.length,'documento','documentos')}</b><small>${esc(collectionOrigins.join(' · '))}</small><div>${pill(status,sameOrigin.length?'ok':'warn')}</div><div class="evidenceDocNames">${sameOrigin.slice(0,4).map(d=>`<span>${esc(d.title||d.document_type||'Documento')} · mesma origem</span>`).join('')}${dateOnly.slice(0,4).map(d=>`<span>${esc(d.title||d.document_type||'Documento')} · apenas mesma data</span>`).join('')}</div></div></article>`;
...
export function renderHealthHub(){
  const labFailed=failed('labs'),docsFailed=failed('docs'),labs=state.data.labs||[],docs=[...(state.data.docs||[])].sort((a,b)=>String(b.document_date||'').localeCompare(String(a.document_date||''))),cols=collections(),groups=biomarkerGroups(),seriesItems=longitudinalGroups(groups),q=norm(state.ui.labQuery);
  ...
  const latestDate=[...labDates].sort().at(-1)||null;
  return `${title('Exames','Evolução laboratorial com origem, unidade e documentos preservados.','Área do histórico')}
    ${labFailed?unavailable('Os exames continuam preservados; tente atualizar para carregar os resultados.') : ''}
    <section class="domainHero"><div><span>Exames · histórico completo</span><h2>${latestDate?`Última coleta em ${fmtDate(latestDate)}`:'Nenhuma coleta estruturada'}</h2><p>${labFailed?'Os resultados não carregaram agora.':`${countLabel(seriesItems.length,'marcador','marcadores')} com série longitudinal segura. Resultados de origens ou unidades diferentes permanecem separados.`}</p></div></section>
    <div class="domainStatStrip sectionGap">
      <div class="card metric"><span>Datas de coleta</span><strong>${labFailed?'—':labDates.length}</strong><em>${labFailed?'não carregado':labDates.length?`${fmtDate([...labDates].sort().at(0))} → ${fmtDate(latestDate)} · ${countLabel(cols.length,'conjunto','conjuntos')}`:'sem datas'}</em></div>
    ...
    <div class="card sectionGap domainChartCard">${labFailed?unavailable('O histórico por marcador ficará disponível após o carregamento dos exames.'):`<div class="cardHead"><div><b>Evolução por marcador</b><small>Selecione uma série. Comparações exigem mesma origem e unidade.</small></div>${pill(countLabel(seriesItems.length,'série','séries'),'ok')}</div>${longitudinalOverview(seriesItems,state.ui.selectedBiomarker)}<div class="labExplorer refined uxLabExplorer"><div class="exerciseList markerList">${filteredGroups.slice(0,80).map(g=>{const s=markerSeriesSummary(g);return`<button type="button" data-marker="${esc(g.key)}" class="${g.key===state.ui.selectedBiomarker?'active':''}"><b>${esc(g.label)}</b><small>${countLabel(g.rows.length,'resultado','resultados')}${s.maxPoints>=2?` · série de até ${s.maxPoints} pontos`:''}</small></button>`;}).join('')||empty('Nenhum marcador encontrado.')}</div><div class="exerciseDetail">${markerHistory(marker)}</div></div>`}</div>
    <details class="uxDisclosure sectionGap"><summary><span><b>Coletas e resultados</b><small>Abra uma coleta para consultar todos os marcadores</small></span><i>Explorar</i></summary><div class="disclosureBody card">${labFailed?unavailable('Não é possível listar coletas enquanto os resultados estão indisponíveis.'):`<div class="cardHead"><div><b>Coleta</b><small>Escolha uma coleta para ver os resultados disponíveis.</small></div><select id="collectionSelect">${cols.map(c=>`<option value="${esc(c.key)}">${fmtDate(c.date)} · ${esc(c.lab)}</option>`).join('')}</select></div><input id="labQuery" class="fullInput" type="search" placeholder="Buscar marcador ou resultado" value="${esc(state.ui.labQuery)}">${collectionPanel(collection)}`}</div></details>
    <details class="uxDisclosure sectionGap"><summary><span><b>Comparar coletas</b><small>Somente a mesma origem e unidades compatíveis</small></span><i>Explorar</i></summary><div class="disclosureBody card">${labFailed?unavailable('A comparação fica disponível quando os resultados carregarem.'):collectionComparison(cols,state.ui.selectedCollection)}</div></details>
    <details class="uxDisclosure sectionGap"><summary><span><b>Documentos e rastreabilidade</b><small>Originais preservados e vínculos por data</small></span><i>Explorar</i></summary><div class="disclosureBody grid cols2"><div class="card"><div class="cardHead"><div><b>Documentos</b><small>Arquivos podem ficar guardados sem gerar resultados até uma leitura segura.</small></div>${!docsFailed?pill(countLabel(docs.length,'documento','documentos')):''}</div>${docsFailed?unavailable('Os documentos não carregaram agora.'):`${documentSummary(docs)}<div class="documentGrid">${docs.slice(0,100).map(d=>`<article class="documentItem"><time>${fmtDate(d.document_date)}</time><div><b>${esc(d.title||d.document_type||'Documento')}</b><small>${esc(d.document_type||'tipo não informado')} · ${esc(d.source||'origem registrada')}</small>${d.source_file?`<em>${esc(d.source_file)}</em>`:''}</div>${documentStatus(d)}</article>`).join('')||empty('Nenhum documento registrado.')}</div>`}</div><div class="card"><div class="cardHead"><div><b>Resultados e documentos na mesma data</b><small>Coincidência de data e mesma origem são estados diferentes.</small></div></div>${labFailed||docsFailed?unavailable('Esta visão precisa dos resultados e dos documentos carregados ao mesmo tempo.'):evidenceByDate(cols,docs)}</div></div></details>
