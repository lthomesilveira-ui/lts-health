...
function sourceState(){
  return [
    {key:'apple_health',name:'Apple Saúde',status:sourceStatusFor('apple_health'),latestDate:latestSourceEvidenceDateFor('apple_health'),capability:'Sincronização automática parcial',readyDetail:'Dados confirmados do Apple Saúde encontrados.',missingDetail:'Conecte o companion ou envie o export do app Saúde.',action:'Enviar Apple Saúde',scope:'Uso automático: energia ativa, minutos de exercício e horas em pé. Passos, FC de repouso, HRV, frequência respiratória, peso e sono podem chegar para validação com origem preservada e não entram automaticamente na visão principal. Sono permanece fora da sincronização automática até existir uma regra validada de escolha entre fontes sobrepostas. Fontes diferentes de sono permanecem separadas.'},
    {key:'polar_flow',name:'Polar Flow',status:sourceStatusFor('polar_flow'),latestDate:latestSourceEvidenceDateFor('polar_flow'),capability:'Apple Saúde em validação + arquivo',readyDetail:'Dados do Polar encontrados.',missingDetail:'Envie um export do Polar Flow ou permita a origem via Apple Saúde.',action:'Enviar Polar',scope:'Dados do Polar presentes no Apple Saúde podem ser preservados por origem para validação. Arquivos podem complementar detalhes de treino; o mesmo treino não é contado duas vezes quando outra fonte já o representa.'},
    {key:'myfitnesspal',name:'MyFitnessPal',status:sourceStatusFor('myfitnesspal'),latestDate:latestSourceEvidenceDateFor('myfitnesspal'),capability:'Export direto + dados via Apple Saúde em validação',readyDetail:'Nutrição estruturada do MyFitnessPal encontrada.',candidateDetail:'Totais diários do MyFitnessPal chegaram pelo Apple Saúde e aguardam validação antes de entrar na alimentação principal.',missingDetail:'Conecte o MyFitnessPal ao Apple Saúde ou envie o export.',action:'Enviar MyFitnessPal',scope:'Quando houver export direto e dados via Apple Saúde, o export direto permanece preferencial. Calorias, proteína, carboidratos, gordura e fibra podem chegar pelo Apple Saúde como totais diários em validação e ficam separados da alimentação principal. Alimentos, refeições e horários não são inventados.'},
    {key:'fleury',name:'Fleury',status:sourceStatusFor('fleury'),latestDate:latestSourceEvidenceDateFor('fleury'),capability:'Arquivo preservado + revisão',readyDetail:'Resultados estruturados do Fleury encontrados.',missingDetail:'Envie CSV, PDF ou imagem do exame.',action:'Enviar Fleury',scope:'CSV, PDF ou imagem são preservados. Resultados só são estruturados quando a leitura for validada; valor, unidade ou faixa ambíguos ficam para revisão.'},
    {key:'einstein',name:'Einstein',status:sourceStatusFor('einstein'),latestDate:latestSourceEvidenceDateFor('einstein'),capability:'Arquivo preservado + revisão',readyDetail:'Resultados estruturados do Einstein encontrados.',missingDetail:'Envie CSV, PDF ou imagem do exame.',action:'Enviar Einstein',scope:'CSV, PDF ou imagem são preservados. Resultados só são estruturados quando a leitura for validada; valor, unidade ou faixa ambíguos ficam para revisão.'}
  ];
}

function statusCard(source){
  const labels={ready:'com dados',candidate:'em validação',processing:'processando',attention:'precisa de atenção',received:'arquivo recebido',unknown:'não verificado',missing:'a importar'};
  const detail=source.status==='ready'?source.readyDetail:source.status==='candidate'?(source.candidateDetail||'Dados desta fonte foram recebidos para validação e permanecem separados da visão principal.'):source.status==='processing'?'Arquivo recebido e ainda em processamento.':source.status==='attention'?'O arquivo foi preservado, mas precisa de revisão.':source.status==='received'?'Há arquivo desta fonte, sem dado estruturado confirmado nesta tela.':source.status==='unknown'?'Não foi possível confirmar esta fonte agora.':source.missingDetail;
  ...
  return `<article class="sourceStatus ${source.status==='ready'?'ready':''}"><div class="sourceStatusTop"><b>${esc(source.name)}</b>${pill(labels[source.status]||'não verificado',kind)}</div><span class="sourceCapability">${esc(source.capability)}</span><p>${esc(detail)}</p>${freshness}<button type="button" data-source-upload="${esc(source.key)}">${esc(source.action)}</button><details class="sourceMore"><summary>Como funciona</summary><small class="sourceScope">${esc(source.scope)}</small></details></article>`;
}
...
const sourceFamilyLabels={apple_activity_summary:'Apple Saúde · ActivitySummary',apple_watch:'Apple Watch',iphone:'iPhone',polar_flow:'Polar Flow',myfitnesspal:'MyFitnessPal',ringconn:'RingConn',healthkit_candidate:'Outras fontes via Apple Saúde'};
...
function provenanceOverview(rows){
  if(failed('sourceMetrics'))return'<div class="errorState"><b>Origem das métricas indisponível agora.</b><span>Os dados confirmados continuam disponíveis. Atualize para tentar carregar as origens novamente.</span></div>';
  const groups=new Map();
  for(const row of rows||[]){const family=String(row.source_family||'unknown');if(!groups.has(family))groups.set(family,{total:0,canonical:0,candidate:0,preserved:0,latest:null});const g=groups.get(family);g.total++;const status=String(row.canonical_status||'').toLowerCase();if(status==='canonical')g.canonical++;else if(status==='candidate')g.candidate++;else g.preserved++;const date=String(row.metric_date||'');if(/^\d{4}-\d{2}-\d{2}$/.test(date)&&(!g.latest||date>g.latest))g.latest=date;}
  const cards=[...groups.entries()].sort((a,b)=>b[1].total-a[1].total||sourceFamilyLabel(a[0]).localeCompare(sourceFamilyLabel(b[0]),'pt-BR')).map(([family,g])=>`<div class="sourceCard provenanceCard"><div><b>${esc(sourceFamilyLabel(family))}</b><small>${g.latest?`dados até ${esc(fmtDate(g.latest))} · `:''}${g.canonical} confirmado(s) · ${g.candidate} em validação${g.preserved?` · ${g.preserved} preservado(s)`:''}</small></div><span>${g.total}</span></div>`).join('');
  return `<div class="sourceGrid provenanceGrid">${cards||'<div class="sourceCard"><div><b>Sem métricas separadas por origem</b><small>Nenhum registro por origem foi carregado.</small></div><span>0</span></div>'}</div><p class="footerNote">A data exibida é a mais recente realmente carregada para cada origem. Só registros marcados explicitamente para validação entram nessa contagem; outras evidências ficam preservadas e separadas dos dados confirmados.</p>`;
}
...
export function renderDataHub(){
  ...
  return `${title('Dados','Importe, acompanhe e faça backup das suas fontes.')}
    ...
    <section class="card sectionGap provenancePanel"><div class="cardHead"><div><b>Origem das métricas</b><small>Veja de onde vieram os registros e se já podem ser usados nas telas principais.</small></div>${failed('sourceMetrics')?pill('indisponível','warn'):pill(`${sourceMetrics.length} registros`)}</div>${provenanceOverview(sourceMetrics)}</section>
    <div class="grid cols2 sectionGap">
