...
function provenanceOverview(rows){
  ...
  for(const row of rows||[]){const family=String(row.source_family||'unknown');if(!groups.has(family))groups.set(family,{total:0,canonical:0,candidate:0,held:0,latest:null});const g=groups.get(family);g.total++;const status=String(row.canonical_status||'candidate').toLowerCase();if(status==='canonical')g.canonical++;else if(status==='candidate')g.candidate++;else g.held++;const date=String(row.metric_date||'');if(/^\d{4}-\d{2}-\d{2}$/.test(date)&&(!g.latest||date>g.latest))g.latest=date;}
  ...
  return `<div class="sourceGrid provenanceGrid">${cards||'<div class="sourceCard"><div><b>Sem métricas por origem</b><small>Nenhum registro de proveniência foi carregado.</small></div><span>0</span></div>'}</div><p class="footerNote">A data exibida é a maior data realmente carregada para cada origem. Candidatos permanecem separados das métricas canônicas e não são somados à Timeline.</p>`;
}
