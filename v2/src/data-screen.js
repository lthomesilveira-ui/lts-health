...
function statusCard(source){
  const labels={ready:'com dados',candidate:'candidatos recebidos',processing:'processando',attention:'precisa de atenção',received:'arquivo recebido',unknown:'não verificado',missing:'a importar'};
  const detail=source.status==='ready'?source.readyDetail:source.status==='candidate'?(source.candidateDetail||'Dados candidatos desta fonte foram recebidos e permanecem separados da visão canônica.'):source.status==='processing'?'Arquivo recebido e ainda em processamento.':source.status==='attention'?'O arquivo foi preservado, mas precisa de revisão.':source.status==='received'?'Há arquivo desta fonte, sem dado estruturado confirmado nesta tela.':source.status==='unknown'?'Não foi possível confirmar esta fonte agora.':source.missingDetail;
...
function provenanceOverview(rows){
  ...
  const cards=[...groups.entries()].sort((a,b)=>b[1].total-a[1].total||sourceFamilyLabel(a[0]).localeCompare(sourceFamilyLabel(b[0]),'pt-BR')).map(([family,g])=>`<div class="sourceCard provenanceCard"><div><b>${esc(sourceFamilyLabel(family))}</b><small>${g.latest?`dados até ${esc(fmtDate(g.latest))} · `:''}${g.canonical} canônico(s) · ${g.candidate} candidato(s)${g.preserved?` · ${g.preserved} preservado(s)`:''}</small></div><span>${g.total}</span></div>`).join('');
  return `<div class="sourceGrid provenanceGrid">${cards||'<div class="sourceCard"><div><b>Sem métricas por origem</b><small>Nenhum registro de proveniência foi carregado.</small></div><span>0</span></div>'}</div><p class="footerNote">A data exibida é a maior data realmente carregada para cada origem. Só registros marcados explicitamente como candidatos entram na contagem de candidatos; demais evidências permanecem preservadas e separadas das métricas canônicas.</p>`;
}
...
export function renderDataHub(){
  ...
  return `${title('Dados','Importe, acompanhe e faça backup das suas fontes.')}
    ...
    <section class="card sectionGap provenancePanel"><div class="cardHead"><div><b>Proveniência das métricas</b><small>Origem, frescor e status dos registros preservados separadamente da visão consolidada.</small></div>${failed('sourceMetrics')?pill('indisponível','warn'):pill(`${sourceMetrics.length} registros`)}</div>${provenanceOverview(sourceMetrics)}</section>
    <div class="grid cols2 sectionGap">
