...
export function renderDataHub(){
  ...
  return `${title('Dados','Traga suas fontes para o mesmo histórico. O app mantém as origens separadas para evitar contagem duplicada.')}
    ...
    <div class="note sectionGap">Cada arquivo permanece ligado à origem. Quando a leitura automática não é segura, o original é preservado e o item fica para revisão em vez de receber dados estimados.</div>
    <section class="backupPanel sectionGap"><div><span>Backup estruturado</span><b>Leve uma cópia dos registros organizados com você.</b><p>O JSON reúne as áreas estruturadas acessíveis à sua sessão e preserva origem e campos ausentes. Arquivos originais privados e credenciais não entram no pacote.</p><small>O arquivo contém dados pessoais de saúde. Guarde-o em local seguro.</small></div><div class="backupActions"><button id="backupExportBtn" type="button" class="primary">Exportar backup</button><span id="backupExportMsg" role="status"></span></div></section>
