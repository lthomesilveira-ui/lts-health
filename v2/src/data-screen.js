...
const metricTopicLabels={sleep_duration_h:'Sono',steps:'Passos',resting_heart_rate_bpm:'Frequência cardíaca em repouso',hrv_sdnn_ms:'Variabilidade da frequência cardíaca',respiratory_rate_bpm:'Frequência respiratória',oxygen_saturation_pct:'Saturação de oxigênio',weight_kg:'Peso',dietary_energy_kcal:'Alimentação',dietary_protein_g:'Alimentação',dietary_carbs_g:'Alimentação',dietary_fat_g:'Alimentação',dietary_fiber_g:'Alimentação',dietary_water_ml:'Hidratação'};
const metricCoverageLabels={...metricTopicLabels,dietary_energy_kcal:'Energia alimentar',dietary_protein_g:'Proteína alimentar',dietary_carbs_g:'Carboidratos alimentares',dietary_fat_g:'Gordura alimentar',dietary_fiber_g:'Fibra alimentar',dietary_water_ml:'Volume de água'};
...
function reviewInbox(uploads,previews,issues,sourceMetrics){
  const uploadCounts=processingCounts(uploads),internalActions=failed('quality')?[]:actionableIssues(issues),uploadActions=failed('uploads')?[]:actionableUploads(uploads),manualCount=uploadActions.length,internalCount=internalActions.length,userActionKnown=!failed('uploads');
  const heldRows=failed('sourceMetrics')?null:reviewRows(sourceMetrics),safeCount=(heldRows?.length||0)+uploadCounts.safeReview;
  const headline=!userActionKnown?'Não foi possível verificar se há ação sua agora':manualCount?`${manualCount} ${manualCount===1?'item realmente precisa':'itens realmente precisam'} de você`:'Nada exige sua ação agora';
  const explanation=!userActionKnown?'Os arquivos continuam preservados. Atualize para verificar se existe alguma pendência que dependa de você.':manualCount?'Só os arquivos abaixo pedem uma ação sua. Pendências abertas de qualidade ficam no tratamento interno do LTS Health e não exigem cliques seus.':internalCount?`Há ${internalCount} ${internalCount===1?'item':'itens'} no tratamento interno do LTS Health. Você não precisa revisar esse backlog; fontes sem regra segura continuam guardadas sem uso automático.`:`O LTS Health está guardando ${safeCount} ${safeCount===1?'registro':'registros'} com cautela. Eles não entram em médias nem são somados entre fontes e você não precisa revisar um por um.`;
  const manualRows=uploadActions.map(upload=>`<div class="reviewActionRow"><div><b>${esc(upload.original_filename||'Arquivo')}</b><small>${esc(sourceLabel(upload.source_type))} · envie novamente se quiser tentar outra leitura.</small></div>${pill('sua atenção','warn')}</div>`);
  const internalRows=internalActions.slice(0,8).map(issue=>`<div class="reviewActionRow"><div><b>${esc(issueTitle(issue))}</b><small>${esc(safeQualityDescription(issue,'Há um ponto no backlog interno de qualidade.'))}</small></div>${pill('tratamento interno')}</div>`);
  const userActionBlock=!userActionKnown?'<div class="errorState"><b>Não foi possível verificar suas pendências de arquivo agora.</b><span>Os arquivos já recebidos continuam preservados; atualize para tentar novamente.</span></div>':manualRows.length?`<div class="reviewActionList" data-user-actions><h3>O que precisa de você</h3>${manualRows.join('')}</div>`:`<div class="reviewSafeNote"><b>Nenhum clique ou decisão sua é necessário agora.</b><span>${internalCount?`Há ${internalCount} ${internalCount===1?'item':'itens'} no backlog interno de qualidade, sem transferir essa revisão para você.`:'O sistema continua preservando as fontes e só vai usar automaticamente o que passar pelas regras de segurança.'}</span></div>`;
  const internalBlock=internalRows.length?`<div class="reviewActionList" data-internal-quality><h3>O que o LTS Health está tratando</h3>${internalRows.join('')}</div>`:'';
  return `<section class="card reviewInbox" data-review-inbox>
    <div class="reviewInboxHero"><div><span class="reviewEyebrow">Conferências</span><h2>${esc(headline)}</h2><p>${esc(explanation)}</p></div>${pill(!userActionKnown?'verificação indisponível':manualCount?'ação necessária':'tudo sob controle',!userActionKnown||manualCount?'warn':'ok')}</div>
    <div class="reviewInboxStats">
      <div class="reviewStat ${userActionKnown&&manualCount?'attention':''}"><span>Sua atenção</span><strong>${userActionKnown?manualCount:'—'}</strong><small>${!userActionKnown?'não foi possível verificar':manualCount?'arquivos que pedem ação':'nenhuma decisão pendente'}</small></div>
      <div class="reviewStat"><span>Tratamento interno</span><strong>${failed('quality')?'—':internalCount}</strong><small>backlog operacional do LTS Health</small></div>
...
function qualityRow(issue,mode){
  ...
  return `<div class="qualityRow"><div><b>${esc(issueTitle(issue))}</b><small>${esc(detail)}</small>${resolution?`<em>${esc(resolution)}</em>`:''}</div>${pill(mode==='resolved'?'resolvido':mode==='known'?'limitação conhecida':'ação interna',mode==='resolved'?'ok':'')}</div>`;
}
...
export function renderDataHub(){
  ...
  return `${title('Dados','Envie arquivos, acompanhe o que já entrou no histórico e veja apenas as conferências que realmente precisam de você.')}
    ...
    <details class="card sectionGap reviewDetails"><summary><span><b>Qualidade e limitações</b><small>Detalhes de ações internas, limitações conhecidas e itens já resolvidos.</small></span><span>Ver detalhes</span></summary><div class="reviewDetailsBody">${qualitySections(issues)}</div></details>`;
}