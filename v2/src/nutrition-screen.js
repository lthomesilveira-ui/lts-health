...
function periodRows(){
  const p=state.ui.nutritionPeriod||'90',all=[...(state.data.nutrition||[])].sort((a,b)=>String(b.nutrition_date).localeCompare(String(a.nutrition_date)));
  if(p==='all')return all;
...
export function renderNutritionHub(){
  ...
  return `${title('Nutrição','Histórico de alimentação registrado. Médias usam somente os dias que possuem dados; dias ausentes não são tratados como zero.')}
    ...
    <div class="card sectionGap"><div class="cardHead"><div><b>Cobertura do histórico</b><small>Quantidade de dias com pelo menos um registro por ano.</small></div></div><div class="yearGrid">${Object.entries(all.reduce((acc,r)=>{const y=String(r.nutrition_date||'').slice(0,4);if(y)acc[y]=(acc[y]||0)+1;return acc;},{})).sort((a,b)=>b[0].localeCompare(a[0])).map(([y,n])=>`<div><b>${esc(y)}</b><span>${n} dias</span></div>`).join('')||empty('Sem histórico anual.')}</div></div>`;
}
