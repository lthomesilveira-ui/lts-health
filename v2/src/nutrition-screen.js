...
const daysInYear=year=>{const y=Number(year);return Number.isFinite(y)?((y%4===0&&y%100!==0)||y%400===0?366:365):0;};
...
function calendarCoverage(rows,period,selectedYear=null){
  const dates=new Set(rows.map(r=>day(r.nutrition_date)).filter(Boolean));
  let expected=0;
  if(selectedYear)expected=daysInYear(selectedYear);
  else if(period==='all'){
...
export function renderNutritionHub(){
  ...
  const all=allNutrition(),spanYears=historyYears(all),available=availableYears(all),coverage=coverageByYear(all),p=state.ui.analysisPeriod||state.ui.nutritionPeriod||'365';
  const selectedYear=p==='all'&&available.includes(String(state.ui.nutritionYear||''))?String(state.ui.nutritionYear):null;
  const baseRows=periodRows(all),rawRows=selectedYear?baseRows.filter(row=>yearOf(row.nutrition_date)===selectedYear):baseRows,dayGroups=nutritionDayGroups(rawRows),rows=unambiguousNutrition(rawRows),calendar=calendarCoverage(rawRows,p,selectedYear);
  ...
  const dayLimit=selectedYear?dayGroups.length:Math.min(30,dayGroups.length),visible=dayGroups.slice(0,dayLimit),coverageText=calendar.expected?`${calendar.recorded} de ${calendar.expected} dias`:calendar.recorded?countLabel(calendar.recorded,'dia registrado','dias registrados'):'sem dias registrados',coverageSub=calendar.expected?`${calendar.pct}% da janela com registro · ${countLabel(calendar.missing,'dia sem registro','dias sem registro')}`:'Sem período comparável';
  const yearSelector=p==='all'&&available.length?`<label>Ano<select id="nutritionYear"><option value="">Todos</option>${available.map(y=>`<option value="${esc(y)}">${esc(y)}</option>`).join('')}</select></label>`:'';
  const selector=`<label>Janela do produto<select id="nutritionPeriod"><option value="30">30 dias</option><option value="90">90 dias</option><option value="365">1 ano</option><option value="all">Todo o histórico</option></select></label>${yearSelector}`;
  const scopeLabel=selectedYear?selectedYear:periodLabel(p);
  if(!dayGroups.length){return `${title('Nutrição','Histórico de alimentação e água, com lacunas mantidas explícitas.')}<section class="domainHero"><div><span>Nutrição · ${esc(scopeLabel)}</span><h2>Nenhum total diário nesta janela</h2><p>${all.length?'Há registros fora deste período. Amplie a janela ou escolha outro ano; ausência não significa consumo zero.':'Ainda não há totais diários estruturados.'}</p></div><div class="controls">${selector}${all.length?'<button type="button" class="primary" data-period="all">Ver todo o histórico</button>':''}</div></section>${hydrationPanel()}`;}
  return `${title('Nutrição','Histórico de alimentação e água. Médias usam somente dias com um total diário inequívoco.')}
    <section class="domainHero"><div><span>Nutrição · ${esc(scopeLabel)}</span><h2>${countLabel(dayGroups.length,'dia registrado','dias registrados')}</h2><p>${selectedYear?`Histórico diário de ${esc(selectedYear)} disponível para navegação completa.`:'Veja a cobertura primeiro e abra refeições ou históricos somente quando precisar.'}</p></div><div class="controls">${selector}</div></section>
    ...
    <div class="grid split sectionGap domainPrimaryGrid">
      ...
      <div class="card"><div class="cardHead"><div><b>${selectedYear?`Dias de ${esc(selectedYear)}`:'Dias recentes'}</b><small>${selectedYear?'Todos os dias registrados do ano selecionado.':'Selecione um dia ou abra um ano para navegar o histórico completo.'}</small></div><span class="pill">${countLabel(dayGroups.length,'dia','dias')}</span></div><div class="nutritionDays">${visible.map(group=>group.ambiguous?`<button type="button" data-nutrition-date="${esc(group.date)}" class="${group.date===state.ui.nutritionDate?'active':''}"><time>${fmtDate(group.date)}</time><div><b>Em revisão</b><small>Mais de um total diário; nenhum valor foi escolhido.</small></div></button>`:`<button type="button" data-nutrition-date="${esc(group.date)}" class="${group.date===state.ui.nutritionDate?'active':''}"><time>${fmtDate(group.date)}</time><div><b>${num(group.row.calories_kcal)!=null?`${fmtNum(group.row.calories_kcal,0)} kcal`:'calorias não registradas'}</b><small>${[num(group.row.protein_g)!=null?`${fmtNum(group.row.protein_g,0)}g proteína`:null,num(group.row.carbs_g)!=null?`${fmtNum(group.row.carbs_g,0)}g carboidratos`:null,num(group.row.fat_g)!=null?`${fmtNum(group.row.fat_g,0)}g gordura`:null].filter(Boolean).join(' · ')}</small></div></button>`).join('')}</div>${!selectedYear&&dayGroups.length>visible.length?`<p class="footerNote">Mostrando ${visible.length} dias recentes. Escolha “Todo o histórico” e um ano para acessar todos os dias preservados.</p>`:''}</div>
    </div>
    ...
    <details class="uxDisclosure sectionGap"><summary><span><b>Distribuição e cobertura histórica</b><small>Refeições registradas e anos com dados</small></span><i>Explorar</i></summary><div class="disclosureBody"><div class="grid split"><div class="card"><div class="cardHead"><div><b>Refeições mais registradas</b><small>Distribuição descritiva das entradas importadas.</small></div></div>${distributionPanel(rawRows)}</div><div class="card"><div class="cardHead"><div><b>Cobertura por ano</b><small>Abra um ano para navegar todos os dias preservados; anos sem registro continuam explícitos.</small></div></div><div class="yearGrid">${spanYears.map(y=>{const c=coverage[y];return c?`<button type="button" data-nutrition-year="${esc(y)}" class="${selectedYear===y?'active':''}"><b>${esc(y)}</b><span>${countLabel(c.count,'dia registrado','dias registrados')}</span><small>${fmtDate(c.first)} → ${fmtDate(c.last)}</small></button>`:`<div class="yearGap"><b>${esc(y)}</b><span>sem registros disponíveis</span><small>Nenhum dia importado neste ano.</small></div>`;}).join('')||empty('Sem histórico anual.')}</div></div></div>${mfpCandidatePanel()}</div></details>`;
}
