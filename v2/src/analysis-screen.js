...
export function renderAnalysisHub(){
  ...
  if(!lf)narrative.push(`${labDates.length} data(s) de coleta laboratorial`);
  const gaps=[];
  if(!lf&&allLabDates.length<2)gaps.push('Exames: há menos de duas datas de coleta estruturadas, então a comparação longitudinal de biomarcadores ainda é limitada.');
  ...
  return `${title('Análise','Uma leitura longitudinal dos registros disponíveis. Relações temporais são contexto, não prova de causa e efeito.')}
    ...
    <div class="grid cols4 sectionGap">
      ...
      ${metric(`Datas de coleta · ${periodText}`,lf?'—':String(labDates.length),lf?'dados indisponíveis agora':rangeOf(labPeriod,'collection_date'))}
    </div>
