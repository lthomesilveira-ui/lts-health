import {state,esc,day,fmtDate,fmtNum,num,norm,unique,countLabel,periodLabel} from './core.js';
...
const periodHeading=period=>period==='30'?'O que aconteceu nos últimos 30 dias':period==='90'?'O que aconteceu nos últimos 90 dias':period==='365'?'O que aconteceu no último ano':'O que aconteceu em todo o histórico';
const periodContext=period=>period==='30'?'nos últimos 30 dias':period==='90'?'nos últimos 90 dias':period==='365'?'no último ano':'em todo o histórico';
...
function labsCard(history,window,period){
  ...
  return`<div class="card"><div class="cardHead"><div><b>Exames</b><small>O histórico não desaparece quando a janela recente não tem coleta.</small></div><span class="pill">última ${fmtDate(history.last)}</span></div><div class="grid cols3 compact sectionGap">${metric('Resultados',String(history.total),'no histórico')}${metric('Datas de coleta',String(history.collections),'no histórico')}${metric('Na janela',String(window.collections),periodLabel(period))}</div><p class="footerNote">${countLabel(history.markers,'marcador estruturado','marcadores estruturados')} no histórico. Comparações detalhadas continuam em Exames quando nome, origem e unidade permitem.</p></div>`;
}
function windowLead({period,bounds,training,nutrition,sleep,protocols}){
  return`<section class="analysisLead"><div class="analysisLeadHead"><div><span>Janela comum</span><h2>${esc(periodHeading(period))}</h2></div><small>Treino, nutrição e recuperação seguem a mesma janela.</small></div><div class="analysisInterval"><b>${bounds.start?fmtDate(bounds.start):'Início do histórico'} → ${bounds.end?fmtDate(bounds.end):'Último registro'}</b><span>Composição e exames mantêm a própria cadência.</span></div><div class="domainStatStrip analysisSecondaryMetrics">${metric('Treinos',training.available?String(training.totalSessions):'—','sessões confirmadas')}${metric('Nutrição',nutrition.available?countLabel(nutrition.days,'dia','dias'):'—','totais diários inequívocos')}${metric('Sono',sleep.available?countLabel(sleep.days,'dia','dias'):'—','evidência preservada')}${metric('Protocolos',String(protocols),'eventos históricos')}</div></section>`;
}
...
function nutritionCard(nutrition){
  ...
  return`<div class="card"><div class="cardHead"><div><b>Nutrição e hidratação</b><small>Médias apenas dos dias estruturados desta janela.</small></div><span class="pill">${countLabel(nutrition.days,'dia','dias')}</span></div><div class="grid cols3 compact sectionGap">${metric('Calorias médias',nutrition.calorieAvg==null?'Sem dado':`${fmtNum(nutrition.calorieAvg,0)} kcal`)}${metric('Proteína média',nutrition.proteinAvg==null?'Sem dado':`${fmtNum(nutrition.proteinAvg,0)} g`)}${metric('Carboidratos médios',nutrition.carbAvg==null?'Sem dado':`${fmtNum(nutrition.carbAvg,0)} g`)}${metric('Gordura média',nutrition.fatAvg==null?'Sem dado':`${fmtNum(nutrition.fatAvg,0)} g`)}${metric('Fibra média',nutrition.fiberAvg==null?'Sem dado':`${fmtNum(nutrition.fiberAvg,0)} g`)}${metric('Água ingerida',water.days&&water.avg!=null?`${fmtNum(water.avg,0)} mL`:'Sem dado',water.days?`${countLabel(water.days,'dia','dias')} com volume`:'nenhum zero é inferido')}</div></div>`;
}
function recoveryCard(sleep,signals,bounds){
  ...
  const originCount=sources||sleep.sources?.length||0;
  return`<div class="card"><div class="cardHead"><div><b>Recuperação</b><small>Sem média entre aparelhos ou origens diferentes.</small></div><span class="pill">${countLabel(sleep.days||0,'dia de sono','dias de sono')}</span></div>${sleep.available?`<div class="grid cols2 compact sectionGap">${metric('Sono estruturado',countLabel(sleep.days,'dia','dias'),`${countLabel(originCount,'origem preservada','origens preservadas')}`) }${metric('Sinais complementares',String(signalRows.length),'séries preservadas na janela')}</div>`:reviewState('Sem sono estruturado','Não há evidência de sono nesta janela.')}${signalRows.length?`<p class="footerNote">Também há ${countLabel(signalRows.length,'série complementar','séries complementares')} preservada${signalRows.length===1?'':'s'} em ${fmtDate(bounds.start)}–${fmtDate(bounds.end)}. Elas permanecem separadas por origem.</p>`:''}</div>`;
}
function digest(body,labs,training,nutrition,sleep,period){
  ...
  cards.push(`<article class="analysisDigestCard"><span>Janela comum</span><h3>${countLabel(training.totalSessions||0,'treino','treinos')}</h3><p>${countLabel(nutrition.days||0,'dia de nutrição','dias de nutrição')} e ${countLabel(sleep.days||0,'dia de sono','dias de sono')} ${periodContext(period)}.</p><button data-route="treinos">Abrir treinos →</button></article>`);
  cards.push(`<article class="analysisDigestCard context"><span>Exames</span><h3>${labs.last?fmtDate(labs.last):'Sem histórico'}</h3><p>${countLabel(labs.total||0,'resultado estruturado','resultados estruturados')} em ${countLabel(labs.collections||0,'data de coleta','datas de coleta')} no histórico.</p><button data-route="saude">Abrir exames →</button></article>`);
...
export function renderAnalysisHub(){
  ...
  return`${title('Recuperação & análises','Uma leitura integrada do que mudou, do que aconteceu na janela e do que ainda limita a análise.','Contexto longitudinal')}<section class="domainHero"><div><span>Análise · ${esc(periodLabel(period))}</span><h2>Três respostas antes dos detalhes</h2><p>Composição e exames usam o histórico comparável; treino, nutrição e recuperação seguem a janela comum.</p></div>${controls}</section>${failureNote}
    ...
    <div class="sectionGap">${windowLead({period,bounds,training,nutrition,sleep,protocols})}</div>
    <div class="sectionGap">${limitations(body,labs,nutrition,sleep)}</div>
    <details class="uxDisclosure sectionGap"><summary><span><b>Como cada área contribui</b><small>Composição, exames, recuperação, nutrição e treino</small></span><i>Explorar</i></summary><div class="disclosureBody"><div class="grid cols2">${bodyCard(body)}${labsCard(labs,labsWindow,period)}</div><div class="grid cols2 sectionGap">${recoveryCard(sleep,signals,bounds)}${nutritionCard(nutrition)}</div><div class="sectionGap">${trainingCard(training,performance)}</div></div></details>
