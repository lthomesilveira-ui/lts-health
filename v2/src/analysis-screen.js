...
function windowLead({period,bounds,training,nutrition,sleep,protocols}){
  return`<section class="analysisLead"><div class="analysisLeadHead"><div><span>Janela recente</span><h2>O que aconteceu nos ${esc(periodLabel(period))}</h2></div><small>Treino, nutrição e recuperação seguem esta janela.</small></div><div class="analysisInterval"><b>${bounds.start?fmtDate(bounds.start):'Início do histórico'} → ${bounds.end?fmtDate(bounds.end):'Hoje'}</b><span>Mesma janela para os sinais de maior frequência.</span></div><div class="grid cols4 analysisSecondaryMetrics">${metric('Treinos',training.available?String(training.totalSessions):'—','sessões confirmadas')}${metric('Nutrição',nutrition.available?`${nutrition.days} dias`:'—','totais diários inequívocos')}${metric('Sono',sleep.available?`${sleep.days} dias`:'—','evidência preservada')}${metric('Protocolos',String(protocols),'eventos históricos')}</div></section>`;
}
