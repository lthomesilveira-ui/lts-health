import {state,esc,num,fmtNum,fmtDate,neutralDelta,bodyRows,norm,day,countLabel} from './core.js';
...
export function renderBioHub(){
  ...
  return `${title('Composição corporal','Evolução das medições corporais, com origem e comparabilidade preservadas.')}
    <section class="domainHero"><div><span>Composição · histórico completo</span><h2>${ambiguousDays.size?'Última medição comparável':'Última medição'} em ${fmtDate(last.measured_at)}</h2><p>${countLabel(allRows.length,'medição preservada','medições preservadas')} · ${esc(sourceDisplay(last.source))}. Os valores são descritivos e não definem meta corporal.</p></div></section>
    ...
    <div class="card sectionGap domainChartCard"><div class="cardHead"><div><b>Evolução corporal</b><small>Escolha uma medida. Datas, escala e valores permanecem explícitos.</small></div><div class="segmented">${Object.entries(metrics).map(([k,m])=>`<button type="button" data-bio-metric="${k}" class="${key===k?'active':''}">${esc(m.label)}${k==='body_fat_pct'?' %':''}</button>`).join('')}</div></div>${lineChart(rows,key,meta.label,meta.unit)}</div>
    <details class="uxDisclosure sectionGap"><summary><span><b>Comparar medições</b><small>Escolha duas datas comparáveis</small></span><i>Explorar</i></summary><div class="disclosureBody grid cols2"><div class="card"><div class="cardHead"><div><b>Diferença entre duas datas</b><small>Comparação descritiva, sem atribuir causa.</small></div></div>${compare(rows)}</div><div class="card"><div class="cardHead"><div><b>Primeiro e último registro</b><small>Somente medições comparáveis.</small></div></div><div class="summaryPair"><div><span>${fmtDate(first.measured_at)}</span><b>${fmtNum(first.weight_kg)} kg</b><small>Massa muscular ${fmtNum(first.skeletal_muscle_mass_kg)} kg</small></div><div class="arrow">→</div><div><span>${fmtDate(last.measured_at)}</span><b>${fmtNum(last.weight_kg)} kg</b><small>Massa muscular ${fmtNum(last.skeletal_muscle_mass_kg)} kg</small></div></div></div></div></details>
    <details class="uxDisclosure sectionGap"><summary><span><b>Visão combinada e histórico</b><small>Quatro séries e todos os registros preservados</small></span><i>Explorar</i></summary><div class="disclosureBody"><div class="card"><div class="cardHead"><div><b>Visão combinada</b><small>Cada medida mantém sua própria escala.</small></div><span class="pill">${countLabel(Math.min(12,rows.length),'medição','medições')}</span></div>${combinedView(rows)}</div><div class="grid split sectionGap"><div class="card"><div class="cardHead"><div><b>Histórico</b><small>Registros ambíguos ficam preservados e fora das comparações.</small></div><span class="pill">${countLabel(allRows.length,'medição','medições')}</span></div><div class="list bodyHistory">${[...allRows].reverse().map(r=>historyRow(r,ambiguousDays)).join('')}</div></div><div class="card bioDetail">${measurementDetail(selected)}<p class="footerNote">Os valores são apresentados de forma descritiva, sem classificação estética ou meta corporal.</p></div></div></div></details>`;
}
