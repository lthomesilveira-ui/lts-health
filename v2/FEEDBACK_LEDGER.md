...
| FB-021 | 15/09 | A versão `.26` aberta no iPhone físico continua grande, recortada e distante da densidade da referência; Home e Treino exibem vazios estruturais. | Tratar as oito capturas físicas como rejeição P0, corrigir a grade da Home, remover reservas duplicadas, compactar o viewport reduzido e transformar `393 × 650` em gate permanente. | Em execução | `PKG-PHYSICAL-IPHONE-REMEDIATION-001`; `v2/physical-iphone-remediation-20260915.css`; `v2/physical-iphone-layout-smoke.mjs`. A promoção e a inspeção pública da `.27` ainda são obrigatórias. |
| FB-022 | 15/09 | Um treino novo precisa entrar no produto sem que uma gravação parcial de dispositivo pareça representar a sessão completa. | Preservar o treino canônico e suas séries no banco privado; apresentar energia extrapolada como estimativa e frequência cardíaca como trecho parcial, mantendo a fonte estruturada. | Em execução | `PKG-LATEST-WORKOUT-TELEMETRY-001`; `v2/src/workout-evidence.js`; `v2/src/training-reference-v2.js`; `v2/workout-source-evidence-smoke.mjs`. |
