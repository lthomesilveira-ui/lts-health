...
async function run(viewport,label){
  ...
  for(const expected of ['Composição corporal','Massa muscular','31 dias entre as medições','+1,0 kg','-1,3 %','+2','InBody'])if(!text.includes(expected))throw new Error(`${label}: initial comparison missing ${expected}`);
  for(const forbidden of ['MME','InBody bioimpedance scan','source_file','confidence'])if(text.includes(forbidden))throw new Error(`${label}: technical body-composition copy visible: ${forbidden}`);
...
console.log('LTS Health v2 body composition comparison smoke passed');