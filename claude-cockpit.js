(function(){
  ...
  function trajectory(body){
    const pairs=[];for(let i=1;i<body.length;i++){const a=body[i],b=body[i-1];pairs.push({a,b})}
    return `<section class="ccTrajectory"><div class="ccSectionHead"><b>Mudança bio-a-bio</b><small>Equivalente estrutural à leitura sequencial do Claude, agora sem rótulos de corpo “bom/ruim”: somente valores e diferenças observadas.</small></div><div class="ccTable"><div class="ccTr head"><span>Data</span><span>MME</span><span>Δ MME</span><span>Gordura</span><span>Δ gordura</span><span>Peso</span><span>Δ peso</span></div>${pairs.slice(-12).reverse().map(({a,b})=>`<div class="ccTr"><time>${fmtDate(a.measured_at)}</time><b>${num(a.skeletal_muscle_mass_kg)!=null?fmtNum(a.skeletal_muscle_mass_kg,1):'—'}</b><em>${diff(a.skeletal_muscle_mass_kg,b.skeletal_muscle_mass_kg,1)}</em><b>${num(a.fat_mass_kg)!=null?fmtNum(a.fat_mass_kg,1):'—'}</b><em>${diff(a.fat_mass_kg,b.fat_mass_kg,1)}</em><b>${num(a.weight_kg)!=null?fmtNum(a.weight_kg,1):'—'}</b><em>${diff(a.weight_kg,b.weight_kg,1)}</em></div>`).join('')||'<div class="ccEmpty">São necessárias pelo menos duas medições.</div>'}</div><p>Os sinais +/− indicam apenas direção matemática entre duas medições consecutivas. Hidratação, método, intervalo e outros fatores podem coexistir; nenhuma classificação automática é aplicada.</p></section>`;
  }
